...
void handleClient(int clientFd) {
    ...
    while (true) {
        ...
        while (true) {
            size_t cmdEnd = command.find('\n');
            if (cmdEnd == std::string::npos) break;

            std::string line = command.substr(0, cmdEnd);
            command.erase(0, cmdEnd + 1);
            ...
            if (line.rfind("UPLOAD ", 0) == 0) {
                ...
                // 写 command 中剩余数据到文件
                fwrite(command.data(), 1, command.size(), file);
                command.clear();

                // 继续接收该文件后续数据直到下一个命令（或 EOF）
                while (true) {
                    ssize_t m = recv(clientFd, buf, sizeof(buf), 0);
                    if (m <= 0) break; // 连接关闭，文件写完
                    std::string chunk(buf, m);

                    // 检查是否新命令开始（有点复杂，这里简化判断）
                    size_t newCmdPos = chunk.find("UPLOAD ");
                    if (newCmdPos != std::string::npos) {
                        fwrite(chunk.data(), 1, newCmdPos, file);
                        command = chunk.substr(newCmdPos); // 留给外层继续处理
                        break;
                    } else {
                ...
                fclose(file);
                std::cout << "文件保存完毕: " << filename << std::endl;
            } else {
