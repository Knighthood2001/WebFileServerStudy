...
void handleClient(int clientFd) {
    ...
    if (command == "EXIT"){
        ...
        // 打开文件准备写入
        std::ofstream outfile(fullpath, std::ios::binary);  //打开服务器端文件，二进制模式打开，用于接收客户端上传的文件数据
        if (!outfile.is_open()) {
        ...
        // 接收文件数据并写入文件
        while (true) {
            ssize_t bytes = recv(clientFd, buffer, BUFFER_SIZE, 0);  // recv函数从客户端接收数据，并将读取的数据存储在buffer中。bytes表示实际读到的字节数
            if (bytes <= 0) break;
